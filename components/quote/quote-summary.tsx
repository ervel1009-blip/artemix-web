...
export function QuoteSummary({ result }: { result: QuoteResult | null }) {
  ...
  const esAMedida = result?.kind === "custom";
  ...
  return (
    <div className="surface-card sticky top-24 overflow-hidden">
      ...
      <div className="relative border-b border-border bg-surface-2/60 px-6 py-5">
        ...
        <div className="relative flex items-center gap-2.5">
          ...
          <h3 className="text-[0.9375rem] font-semibold">
            {esAMedida ? "Tu proyecto" : "Estimación en vivo"}
          </h3>
        </div>
      </div>
      ...
      <div className="px-6 py-6">
        {/*
          AnimatePresence recibe UN solo hijo cuya key cambia con el estado.
          Con varios hermanos condicionales el saliente puede quedarse montado
          y superponer dos estados en el panel.
        */}
        <AnimatePresence mode="wait">
          <motion.div
            key={result?.kind ?? "empty"}
          ...
          >
            {result?.kind === "estimate" ? (
              ...
              <div className="py-6 text-center">
                <Calculator
                  className="mx-auto h-9 w-9 text-faint/40"
                  strokeWidth={1.2}
                />
...
function Plazo({ weeks }: { weeks: [number, number] }) {
  return (
    <p className="mt-5 flex items-center gap-2 text-sm text-muted">
      <Clock3 className="h-4 w-4 text-accent" />
      Tiempo estimado:{" "}
      <span className="font-semibold text-text">
        {weeks[0]}–{weeks[1]} semanas
      </span>
    </p>
  );
}
